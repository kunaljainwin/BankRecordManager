...
struct tagDemat
{
    char lcNumber[16+1];
    tagDemat(){
        strcpy_s(lcNumber,"xxxxxxxxxxxxxxxx");
    }
    void Create(){
        string lstrTemp;
        do{
        ...
        strcpy_s(lcNumber,lstrTemp.c_str());
...
struct tagFixedDeposit
{
    ...
    float lfDepositAmount[5];
    tagFixedDeposit(){
        lnCount=0;
        float lfDepositAmount[5]={0.0};
    }
    void Create(){
       string count;
        do{
            cout << "\nPlease Enter the Number of Fixed Deposits: ";
        cin >> count;
        }
        while(!(isValidAmount(count)&&stoi(count)>0&&stoi(count)<5));
        lnCount=stoi(count);
        for(int i=0;i<lnCount;i++){
            ...
            lfDepositAmount[i]=stof(strAmount);
        }
        
    }
    void Display(){
        cout << "\nFixed Deposit Amounts ("<<lnCount<<"): "<<endl;
        for(int i=0;i<lnCount;i++){
            cout << " " << lfDepositAmount[i];
        }
    }
    void Modify(){
        Create();
...
{
    unsigned int lnCount;
    float lfPolicyCoverage[5];
    tagLifeInsurance(){
        lnCount=0;
        lfPolicyCoverage[5]={0};
    }
    void Create(){
        // not taking count input
        string count;
        do{
            cout << "\nPlease Enter the Number of Policies: ";
        cin >> count;
        }
        while(!(isValidAmount(count)&&stoi(count)<=5&&stoi(count)>0));
        ...
        for(int i=0;i<lnCount;i++){
            ...
            while(isValidAmount(strAmount)==false);
            lfPolicyCoverage[i]=stof(strAmount);
        }
    }
    void Display(){
        cout << "\nLife Insurance Policy Coverage: ("<<lnCount<<")"<<endl;
        for(int i=0;i<lnCount;i++){
            cout << lfPolicyCoverage[i] <<"  ";
        }
    }
    void Modify(){
        Create();
    ...
    void Create(){
       ...
       while(lnCount<1 || lnCount>3);
        for(int i=0;i<lnCount;i++){
            ...
            strcpy_s(lcName[i],strName.c_str());
        }
    }
    void Display(){
        for(int i=0;i<lnCount;i++){
...
struct tagRecord{
    ...
    map<unsigned short int ,void *>mapService;
    void CreateRecord(){
    string lstrTemp;
    nServiceFlag+=ACTIVE;
   ...
   strcpy_s(sFirstName,lstrTemp.c_str());
    ...
    strcpy_s(sLastName,lstrTemp.c_str());
    ...
    strcpy_s(sMobile,lstrTemp.c_str());
 ...
 cin.clear();
    ...
    strcpy_s(sEmailId,lstrTemp.c_str());
...
cout<<"\n2.Current Account";
           ...
           cout << "\nDo you want to avail Saving Account? (Y/N): ";
            while (true)
            {
                ...
                if (lstrTemp[0] == 'Y' || lstrTemp[0]== 'y' )
                {
                    ...
                    if(!(nServiceFlag & SAVINGS_ACCOUNT)){
            ...
            if(!(nServiceFlag & CURRENT_ACCOUNT)){
    ...
    //    cin.ignore(numeric_limits<streamsize>::max(), '\n');
          ...
          tagMedicalInsurance*ptr=new tagMedicalInsurance;
                   ...
                   if(!(nServiceFlag & MEDICAL_INSURANCE)){
           ...
           \
              ...
              while(true){
                ...
                if (lstrTemp[0] == 'Y' || lstrTemp[0] == 'y')
                {
                   ...
                   if(!(nServiceFlag & LIFE_INSURANCE)){
    ...
    void DisplayRecord(){
        ...
        if((nServiceFlag&SAVINGS_ACCOUNT)==SAVINGS_ACCOUNT)
        {
            ...
            cout << "\nService Type: Savings Account";
           ((tagSavingAccount*)mapService[SAVINGS_ACCOUNT])->Display();
        }
         if((nServiceFlag&CURRENT_ACCOUNT)==CURRENT_ACCOUNT)
        {
              cout << "\nService Type: Current Account";
